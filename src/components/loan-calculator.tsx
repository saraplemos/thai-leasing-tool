...
export function LoanCalculator() {
  ...
  const calc = useMemo(() => {
    ...
    if (budgetMode) {
      ...
      return { monthly: budget, principal, totalInterest: total - principal, totalPayment: total };
    }
    const monthly = (amount * r) / (1 - Math.pow(1 + r, -term));
    const totalPayment = monthly * term;
    return { monthly, principal: amount, totalInterest: totalPayment - amount, totalPayment };
  ...
  const applySearch = {
    amount: budgetMode ? Math.round(calc.principal) : amount,
    term,
    rate,
  ...
  return (
    <div className="bg-card rounded-2xl p-6 sm:p-8 shadow-[var(--shadow-elevated)] border border-border w-full">
      <div className="flex items-start justify-between gap-4 mb-6">
        <h3 className="text-xl sm:text-2xl font-bold text-foreground">Calculate your monthly payment</h3>
        <span className="hidden sm:inline-flex shrink-0 text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded bg-accent text-accent-foreground">Live</span>
      </div>

      <div className="flex p-1 bg-muted rounded-lg mb-6 text-xs font-semibold">
        <button onClick={() => setBudgetMode(false)} className={`flex-1 py-2 rounded-md transition ${!budgetMode ? "bg-card shadow-sm text-foreground" : "text-muted-foreground"}`}>By loan amount</button>
        <button onClick={() => setBudgetMode(true)} className={`flex-1 py-2 rounded-md transition ${budgetMode ? "bg-card shadow-sm text-foreground" : "text-muted-foreground"}`}>I know my budget</button>
      </div>
      ...
      <div className="space-y-5">
        {!budgetMode ? (
          <div>
            ...
            <div className="flex justify-between mt-2 text-[11px] text-muted-foreground"><span>100,000</span><span>10,000,000</span></div>
          </div>
        ) : (
          <div>
            <div className="flex items-baseline justify-between mb-1">
            ...
            <div className="flex justify-between mt-2 text-[11px] text-muted-foreground"><span>3,000</span><span>150,000</span></div>
          </div>
        )}
        ...
        <div>
          ...
          <div className="flex justify-between mt-2 text-[11px] text-muted-foreground"><span>12</span><span>84</span></div>
        </div>
        ...
        <div>
          ...
          <div className="flex justify-between mt-2 text-[11px] text-muted-foreground"><span>7.03%</span><span>14.69%</span></div>
        </div>
      </div>
      ...
      <div className="mt-6 rounded-xl bg-muted/60 border border-border p-5">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          <div>
            <div className="text-xs uppercase tracking-wider text-muted-foreground font-semibold">Estimated Monthly Payment</div>
            <div className="mt-1 flex items-baseline gap-2">
              <span className="text-4xl sm:text-5xl font-black text-primary tracking-tight">{formatTHB(calc.monthly)}</span>
              <span className="text-sm font-medium text-muted-foreground">THB</span>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-x-6 gap-y-1.5 text-xs">
            <span className="text-muted-foreground">Total Interest</span>
            <span className="text-right font-semibold text-foreground">{formatTHB(calc.totalInterest)} THB</span>
            <span className="text-muted-foreground">Total Payment</span>
            <span className="text-right font-semibold text-foreground">{formatTHB(calc.totalPayment)} THB</span>
            {!budgetMode && (
              <>
                <span className="text-muted-foreground">{altTerm} months</span>
                <span className="text-right font-semibold text-foreground/70">{formatTHB(altMonthly)} /mo</span>
              </>
            )}
          </div>
        </div>
      ...
      <p className="mt-4 text-[11px] text-muted-foreground text-center">No obligation · Secure & confidential · Estimate for reference only</p>
    </div>
  );
}
